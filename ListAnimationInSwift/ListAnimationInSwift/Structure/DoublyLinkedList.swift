//
//  DoublyLinkedList.swift
//  ListAnimationInSwift
//
//  Created by 任俊峰 on 2019/2/25.
//  Copyright © 2019 任俊峰. All rights reserved.
//

import Foundation
class DoublyLinkedListNode {
    var data: AnyObject
    var next: DoublyLinkedListNode?
    var pre: DoublyLinkedListNode?
    
    init(data: AnyObject = "" as AnyObject) {
        self.data = data
    }
    
    deinit{
        print("\(self.data)释放", separator: "", terminator: ",")
    }
}

class DoublyLinkedList: ListProtocalType {
    var headNode: DoublyLinkedListNode?
    var tailNode: DoublyLinkedListNode?
    var length: UInt = 0
    
    init() {
        self.initHeadAndTailNode()
    }
    
    func initHeadAndTailNode() {
        self.headNode = DoublyLinkedListNode()
        self.tailNode = self.headNode
    }
    
    func count() -> UInt {
        return length
    }
    
    // MARK: - 链表的创建
    /**
     根据数组正向创建链表, 链表的顺序与数组的顺序一致
     
     - parameter items: 数组
     
     - returns: true-创建成功， false-创建失败
     */
    func forwardDirectionCreateList(items: Array<AnyObject>) -> Bool {
        for item in items {
            if !self.addItemToTail(item: item) {
                return false
            }
        }
        return true
    }
    
    /**
     
     根据数组逆向创建数组，链表的顺序与数组的顺序相反
     - parameter items: 数组
     
     - returns: true-创建成功， false-创建失败
     */
    func reverseDirectionCreateList(items: Array<AnyObject>) -> Bool {
        for item in items {
            if !self.addItemToHead(item: item) {
                return false
            }
        }
        return true
    }
    
    
    // MARK: - 链表元素的增加
    /**
     往链表前方追加元素
     
     - parameter item: 所追加的元素
     
     - returns: true-追加成功，false-追加失败
     */
    func addItemToTail(item: AnyObject) -> Bool {
        let newLinkListNode = DoublyLinkedListNode(data: item)
        if self.tailNode == nil {
            return false
        }
        
        //链表关联
        self.tailNode?.next = newLinkListNode
        newLinkListNode.pre = self.tailNode
        
        self.tailNode = newLinkListNode
        self.length += 1
        return true
    }
    
    
    /**
     往链表后方追加元素
     
     - parameter item: 所追加的元素
     
     - returns: true-追加成功，false-追加失败
     */
    func addItemToHead(item: AnyObject) -> Bool {
        let newLinkListNode = DoublyLinkedListNode(data: item)
        if self.headNode == nil {
            return false
        }
        
        newLinkListNode.next = headNode?.next
        if headNode?.next != nil {
            headNode?.next?.pre = newLinkListNode
        }
        
        self.headNode?.next = newLinkListNode
        newLinkListNode.pre = headNode
        
        self.length += 1
        if length == 1 {
            self.tailNode = newLinkListNode;
        }
        return true
    }
    
    /**
     根据指定索引来插入item
     
     - parameter item:  插入链表中的元素
     - parameter index: 要插入的位置（0-length）
     
     - returns: true-插入成功，false-插入失败
     */
    func insertItem(item: AnyObject, index: UInt) -> Bool {
        if !checkIndex(index: index) {
            return false
        }
        
        if index == 0 {
            return self.addItemToHead(item: item)
        }
        
        if index == self.length {
            return self.addItemToTail(item: item)
        }
        
        var cursor = self.headNode
        for _ in 0..<index {
            cursor = cursor?.next
        }
        
        let newItme = DoublyLinkedListNode(data: item)
        
        newItme.next = cursor?.next
        if cursor?.next != nil{
            cursor?.next?.pre = newItme
        }
        
        cursor?.next = newItme
        newItme.pre = cursor
        
        self.length += 1
        
        return true
    }
    
    
    //MARK: - 链表元素的移除
    /**
     正向移除链表中所有的数据
     */
    func removeAllItemFromHead() {
        while self.removeFirstNode() != nil {}
    }
    
    /**
     逆向移除链表中所有的数据
     */
    func removeAllItemFromLast() {
        while self.removeLastNode() != nil {}
    }
    
    
    /**
     移除第一个元素
     
     - returns: 移除成功就返回节点数据，如果移除失败则返回nil
     */
    func removeFirstNode() -> AnyObject? {
        if self.headNode?.next == nil {
            print("链表为空")
            return nil                    //链表为空
        }
        
        let removeItem = self.headNode?.next
        
        self.headNode?.next = removeItem?.next
        if removeItem?.next != nil {
            removeItem?.next?.pre = headNode
        }
        
        removeItem?.next = nil
        removeItem?.pre = nil
        self.length -= 1
        
        if self.headNode?.next == nil {     //如果移除的是最后一个元素，就讲尾指针指向头指针
            self.tailNode = self.headNode
        }
        return removeItem?.data
    }
    
    /**
     移除最后一个节点
     
     - returns: 被移除的节点值
     */
    func removeLastNode() -> AnyObject? {
        if self.headNode?.next == nil {
            print("链表为空")
            return nil                    //链表为空
        }
        
        let preNode = self.tailNode?.pre
        let removeItem = self.tailNode
        
        removeItem?.next = nil
        removeItem?.pre = nil
        preNode?.next = nil
        
        self.tailNode = preNode
        self.length -= 1
        
        return removeItem?.data
    }
    
    /**
     根据索引移除节点
     
     - parameter index: 索引
     
     - returns: 被移除的节点值
     */
    func removeItme(index: UInt) -> AnyObject? {
        if self.headNode?.next == nil {
            print("链表为空")
            return nil                    //链表为空
        }
        
        if !self.checkIndex(index: index) {
            return nil
        }
        
        var cursor = self.headNode      //遍历节点的游标
        
        for i in 0..<self.length {      //寻找移除的节点的位置，以及前驱
            cursor = cursor?.next
            if index == i {
                break
            }
        }
        
        let preCursor = cursor?.pre   //记录一下cursor前面的节点
        preCursor?.next = cursor?.next
        
        if cursor?.next != nil {
            cursor?.next?.pre = preCursor
        }
        
        cursor?.next = nil
        cursor?.pre = nil
        
        if index == self.length-1 {
            self.tailNode = preCursor
        }
        self.length -= 1;
        
        return cursor?.data;
    }
    
    //MARK: 链表的查询
    /**
     根据索引返回值
     */
    public func getNode(atIndex index: Int) -> AnyObject? {
        if index >= 0 {
            var node = headNode
            var i = index
            while node != nil {//从首元结点开始向后查找
                if i == 0 {
                    return node?.data
                }
                i -= 1
                node = node!.next
            }
        }
        return nil
    }
    
    
    //MARK: 链表的遍历
    /**
     单向链表的遍历
     */
    func display() {
        var currentNode = self.headNode?.next
        for _ in 0..<self.length {
            if currentNode == nil {
                break
            }
            guard let item = currentNode?.data else {
                return
            }
            print(item, separator: "", terminator: " <-> ")
            currentNode = currentNode?.next
        }
        print("nil")
    }
    
    // MARK: - 数据验证
    /**
     检查index是否合法
     
     - parameter index: 索引
     
     - returns: true合法，false不合法
     */
    func checkIndex(index: UInt) -> Bool {
        if  index > self.length  {
            print("index非法，请进行检查")
            return false
        }
        return true
    }
    
    
    
    
}


